...
router.get("/public/conversations/:conversationId/messages", async (req, res) => {
  try {
    ...
    const [conv] = await db.select({ id: conversationsTable.id }).from(conversationsTable).where(eq(conversationsTable.id, conversationId)).limit(1);
...
const WIDGET_LOADER_JS = `(function(){
  ...
  btn.style.cssText = "position:fixed;bottom:24px;right:24px;width:60px;height:60px;border-radius:50%;background:" + color + ";display:flex;align-items:center;justify-content:center;cursor:pointer;z-index:2147483646;box-shadow:0 4px 20px rgba(0,0,0,0.25);transition:transform 0.2s;";
