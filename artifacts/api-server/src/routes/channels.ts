...
router.post("/:id/connect", requireAuth, async (req, res) => {
  try {
    ...
    const [channel] = await db.select().from(channelConnectionsTable)
      .where(and(eq(channelConnectionsTable.id, req.params.id), eq(channelConnectionsTable.storeId, storeId))).limit(1);
    ...
    const [updated] = await db.update(channelConnectionsTable)
      ...
      .where(eq(channelConnectionsTable.id, req.params.id))
      .returning();
